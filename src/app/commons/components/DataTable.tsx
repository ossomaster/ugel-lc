...
const DataTable = <T,>({
  ...
  const exportableColumns = useMemo(
    () => columns.filter((col) => col.selector),
    [columns],
  );
  ...
  const getHeaders = useCallback(
    () =>
      exportableColumns.map((col) =>
        typeof col.name === "string" ? col.name : String(col.id || ""),
      ),
    [exportableColumns],
  );
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((row) =>
      exportableColumns.some((col) =>
        String(col.selector!(row, 0)).toLowerCase().includes(lower),
      ),
    );
  ...
  const exportToPdf = async () => {
    ...
    const headers = getHeaders();
    const rows = getExportRows(filteredData);
    const widths = exportableColumns.map((col) =>
      col.grow && col.grow > 1 ? "*" : "auto",
    );
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      <RDTDataTable
        ...
        noDataComponent={
          <div className="py-8 text-gray-500">{noDataMessage}</div>
        }
